...
- Precisa das dependências do dispositivo (caixa/requirements.txt) no mesmo ambiente do servidor.
"""
...
if config.CAIXA_DIR not in sys.path:  # o modo teste importa o vision/ que fica em caixa/
    sys.path.insert(0, config.CAIXA_DIR)
...
def availability() -> tuple[bool, str | None]:
    ...
    if missing:
        return False, ("Este servidor não tem as dependências do dispositivo (" + ", ".join(missing)
                       + "). Para testar aqui: pip install -r caixa/requirements.txt")
    if not (Path(config.CAIXA_DIR) / "vision" / "models" / "face_landmarker.task").is_file():
        return False, "Modelo do rosto não encontrado em caixa/vision/models."
    return True, None
...
class AnalysisJobs:
    ...
    def _process(self, job_id: str) -> None:
        ...
        with self._queue_lock:  # um vídeo por vez: a análise usa bastante CPU
            ...
            self._save(job)
            command = [sys.executable, str(Path(config.FERRAMENTAS_DIR) / "analisar_video.py"), str(self.video_path(job)),
                       "--saida", str(output)]
            if job["phone"]:
